...
# wget https://huggingface.co/datasets/Arabic-Clip/Arabic_MSCOCO_training/blob/main/Arabic_MSCOCO_training.csv
dataset_csv_mscoco = pd.read_csv("Arabic_MSCOCO_training.csv")


#%%
# dataset_csv_mscoco = dataset_csv[3000000:5000000]
...
jsonl_file_path = 'dataset_csv_mscoco.jsonl'

tot = len(dataset_csv_mscoco)

# Convert the DataFrame to JSONL
with open(jsonl_file_path, 'w') as jsonl_file:
    for _, row in tqdm(dataset_csv_mscoco.iterrows(), total=tot):
...
for idx_, list_captions in tqdm(enumerate(json_objects), total=totoal_tqdm):
    ...
    idx_list.append(list_captions['id'])
...
# load the model and tokenizer to compute the embeddings
...
with open("processed_dataset_csv_mscoco_train_v2.jsonl",'w') as file:
...
with open("processed_dataset_csv_mscoco_train_v2.jsonl",'r') as file:
